...
HMENU g_hMMEMenu=NULL;
HMENU g_hMMELanguageMenu=NULL;
bool g_isMMELanguageMenuAttached = false;
HWND g_hFrame = NULL; //フレーム数
HWND g_hMMEDialog = NULL;
HWND g_hMMETab = NULL;
HWND g_hMMEList = NULL;
LONG_PTR g_originalMMEDialogProc = NULL;
LONG_PTR g_originalMMETabProc = NULL;
LONG_PTR g_originalMMEListProc = NULL;
extern HINSTANCE hInstance;

namespace
{
	const UINT IDM_MME_OPEN_OBJECT = 1902;
	const UINT IDM_MME_OPEN_EFFECT = 1903;
	const UINT IDC_MENU_MME_LANGUAGE_ZH_CN = 1904;
	const UINT IDC_MENU_MME_LANGUAGE_SOURCE = 1905;
	const UINT IDC_MME_TAB = 1002;
	const UINT IDC_MME_LIST = 1003;
	const UINT IDC_MME_UPDATE = 1005;
	const wchar_t* k_mme_dialog_class = L"#32770";
	const wchar_t* k_mme_list_class = L"SysListView32";
	const wchar_t* k_mme_tab_class = L"SysTabControl32";
	const wchar_t* k_mme_subset_prefix1 = L"  |-- ";
	const wchar_t* k_mme_subset_prefix2 = L"  `-- ";
	const wchar_t* k_mme_default_label = L"(default)";
	const wchar_t* k_mme_none_label = L"(none)";
	const wchar_t* k_mme_hide_label = L"(hide)";
	const wchar_t* k_mme_invalid_label = L"(invalid)";

	enum class MMEUiLanguage
	{
		Source,
		Chinese
	};
	...
	struct MMEUiText
	{
		const wchar_t* main_menu_title;
		const wchar_t* menu_effect_mapping;
		const wchar_t* menu_enable_effect;
		const wchar_t* menu_auto_reload;
		const wchar_t* menu_auto_save_mapping;
		const wchar_t* menu_reload_all;
		const wchar_t* menu_log_window;
		const wchar_t* menu_about;
		const wchar_t* menu_language;
		const wchar_t* menu_language_chinese;
		const wchar_t* menu_language_source;
		const wchar_t* popup_root_file;
		const wchar_t* popup_root_edit;
		const wchar_t* popup_root_tool;
		const wchar_t* popup_open;
		const wchar_t* popup_save;
		const wchar_t* popup_open_by_model;
		const wchar_t* popup_save_by_model;
		const wchar_t* popup_close;
		const wchar_t* popup_reset_default;
		const wchar_t* popup_subset_extract;
		const wchar_t* popup_disassemble;
		const wchar_t* popup_skip_validation;
		const wchar_t* dialog_title;
		const wchar_t* main_render_target;
		const wchar_t* select_file;
		const wchar_t* remove;
		const wchar_t* hide_show;
		const wchar_t* bulk_action;
		const wchar_t* ok;
		const wchar_t* cancel;
		const wchar_t* refresh;
		const wchar_t* language_toggle;
		const wchar_t* open_object;
		const wchar_t* open_effect;
		const wchar_t* open_object_error;
		const wchar_t* open_effect_error;
	};

	MMEUiLanguage g_mme_ui_language = MMEUiLanguage::Source;
	...
	static std::wstring wide_from_ansi_path(const char* path)
	{
		if (!path || !(*path))
		{
			return std::wstring();
		}
		const int size = ::MultiByteToWideChar(CP_ACP, 0, path, -1, NULL, 0);
		if (size <= 0)
		{
			return std::wstring();
		}
		std::vector<wchar_t> buffer(size, 0);
		::MultiByteToWideChar(CP_ACP, 0, path, -1, &buffer[0], size);
	...
	static std::wstring get_file_name(const std::wstring& path)
	{
		...
		return file ? std::wstring(file) : std::wstring();
	}

	static std::wstring get_file_stem(const std::wstring& path)
	{
		std::wstring filename = get_file_name(path);
		const size_t pos = filename.find_last_of(L'.');
		return (pos == std::wstring::npos) ? filename : filename.substr(0, pos);
	}

	static std::wstring trim_text(const std::wstring& text)
	{
	...
	static bool starts_with(const std::wstring& text, const wchar_t* prefix)
	{
	...
	static std::wstring strip_subset_prefix(const std::wstring& text)
	{
		if (starts_with(text, k_mme_subset_prefix1))
		{
			return trim_text(text.substr(wcslen(k_mme_subset_prefix1)));
		}
		if (starts_with(text, k_mme_subset_prefix2))
		{
			return trim_text(text.substr(wcslen(k_mme_subset_prefix2)));
		}
		return trim_text(text);
	}

	static bool is_subset_item_text(const std::wstring& text)
	{
		return starts_with(text, k_mme_subset_prefix1) || starts_with(text, k_mme_subset_prefix2);
	}

	static bool equals_ci(const std::wstring& lhs, const std::wstring& rhs)
	{
	...
	static std::wstring get_window_text_w(HWND hWnd)
	{
		...
		::GetWindowTextW(hWnd, &buffer[0], size + 1);
		return std::wstring(&buffer[0]);
	}

	static std::wstring get_list_view_item_text(HWND hList, int index, int subitem)
	{
		wchar_t buffer[1024] = {};
		LVITEMW item = {};
		...
		::SendMessageW(hList, LVM_GETITEMTEXTW, index, reinterpret_cast<LPARAM>(&item));
		return std::wstring(buffer);
	}

	static std::wstring get_list_view_item_text(HWND hList, int index)
	{
		return get_list_view_item_text(hList, index, 0);
	}

	static int get_list_view_column_count(HWND hList)
	{
		HWND header = ListView_GetHeader(hList);
		return header ? Header_GetItemCount(header) : 1;
	}

	static const MMEUiText& current_mme_ui_text()
	{
		static const MMEUiText source = {
			L"MMEffect",
			L"エフェクト割当(&M)",
			L"エフェクト使用(&E)\tCtrl+Shift+E",
			L"自動更新(&O)",
			L"割当ファイル自動保存(&S)",
			L"全て更新(&R)",
			L"ログウインドウ(&L)",
			L"MMEffectのバージョン情報(&A)",
			L"言語",
			L"中国語",
			L"日本語（原文）",
			L"ファイル(&F)",
			...
			L"打开 Object 所在位置",
			L"打开 Effect 所在位置",
			L"現在の項目から Object の元ファイルを特定できません。",
			L"現在の項目から Effect ファイルを特定できません。"
		};
		static const MMEUiText chinese = {
			L"MM效果",
			L"效果文件分配(&M)",
			L"启用效果(&E)\tCtrl+Shift+E",
			L"自动重载(&O)",
			L"自动保存映射文件(&S)",
			L"全部重载(&R)",
			L"日志窗口(&L)",
			...
			L"打开 Object 所在位置",
			L"打开 Effect 所在位置",
			L"无法根据当前项目定位 Object 源文件。",
			L"无法根据当前项目定位 Effect 文件。"
		};
		return g_mme_ui_language == MMEUiLanguage::Chinese ? chinese : source;
	}

	static std::vector<MMEItemPath> collect_mme_item_paths()
	{
		std::vector<MMEItemPath> items;
		items.reserve(ExpGetPmdNum() + ExpGetAcsNum());
		for (int i = 0; i < ExpGetPmdNum(); ++i)
		{
			const char* filename = ExpGetPmdFilename(i);
			std::wstring path = wide_from_ansi_path(filename);
			if (!path.empty())
			{
				MMEItemPath item;
				item.path = path;
				item.filename = get_file_name(path);
				item.stem = get_file_stem(path);
				items.push_back(item);
			}
		}
		for (int i = 0; i < ExpGetAcsNum(); ++i)
		{
			const char* filename = ExpGetAcsFilename(i);
			std::wstring path = wide_from_ansi_path(filename);
			if (!path.empty())
			{
				MMEItemPath item;
	...
	static std::wstring resolve_selected_mme_item_path(HWND hList)
	{
		if (!hList) return std::wstring();

		const int selected = static_cast<int>(::SendMessageW(hList, LVM_GETNEXTITEM, -1, LVNI_SELECTED));
		if (selected < 0) return std::wstring();

		std::wstring selected_text = trim_text(get_list_view_item_text(hList, selected));
		if (selected_text.empty()) return std::wstring();
		if (equals_ci(selected_text, k_mme_default_label)) return std::wstring();

		if (is_subset_item_text(selected_text))
		{
			for (int i = selected - 1; i >= 0; --i)
			{
				std::wstring parent_text = trim_text(get_list_view_item_text(hList, i));
				if (parent_text.empty()) continue;
				if (equals_ci(parent_text, k_mme_default_label)) break;
				if (!is_subset_item_text(parent_text))
				{
					selected_text = parent_text;
					break;
				}
			}
		}

		selected_text = strip_subset_prefix(selected_text);
		if (selected_text.empty()) return std::wstring();

		std::vector<MMEItemPath> items = collect_mme_item_paths();
		for (size_t i = 0; i < items.size(); ++i)
		{
			const MMEItemPath& item = items[i];
			if (equals_ci(selected_text, item.filename) || equals_ci(selected_text, item.stem))
			{
				return item.path;
			}
		}
	...
	static bool is_effect_placeholder(const std::wstring& text)
	{
		return text.empty()
			|| equals_ci(text, k_mme_none_label)
			|| equals_ci(text, k_mme_hide_label)
			|| equals_ci(text, k_mme_default_label)
			|| equals_ci(text, k_mme_invalid_label);
	}

	static std::wstring get_effect_candidate_from_selection(HWND hList)
	{
		if (!hList) return std::wstring();
		const int selected = static_cast<int>(::SendMessageW(hList, LVM_GETNEXTITEM, -1, LVNI_SELECTED));
		if (selected < 0) return std::wstring();

		const int column_count = std::max(1, get_list_view_column_count(hList));
		for (int subitem = 1; subitem < column_count; ++subitem)
		{
			std::wstring text = trim_text(get_list_view_item_text(hList, selected, subitem));
			if (is_effect_placeholder(text)) continue;
			if (text.find(L".fx") != std::wstring::npos || text.find(L".FX") != std::wstring::npos)
			{
				return text;
			}
		}
	...
	static std::wstring get_directory_path(const std::wstring& path)
	{
		if (path.empty()) return std::wstring();
		wchar_t folder[MAX_PATH] = {};
		wcsncpy_s(folder, path.c_str(), _TRUNCATE);
		::PathRemoveFileSpecW(folder);
		return std::wstring(folder);
	}

	static std::wstring resolve_effect_path(HWND hList)
	{
		const std::wstring effect = get_effect_candidate_from_selection(hList);
		if (is_effect_placeholder(effect)) return std::wstring();

		if (::PathFileExistsW(effect.c_str()))
		{
			return effect;
		}

		const std::wstring object_path = resolve_selected_mme_item_path(hList);
		const std::wstring object_dir = get_directory_path(object_path);
		const std::wstring base_dir = BridgeParameter::instance().base_path;
		std::wstring filename = get_file_name(effect);
		if (filename.empty())
		{
			filename = effect;
		}
		...
		const std::wstring candidates[] = {
			object_dir.empty() ? std::wstring() : object_dir + L"\\" + effect,
			object_dir.empty() ? std::wstring() : object_dir + L"\\" + filename,
			base_dir.empty() ? std::wstring() : base_dir + effect,
			base_dir.empty() ? std::wstring() : base_dir + filename
		};
		for (size_t i = 0; i < sizeof(candidates) / sizeof(candidates[0]); ++i)
		{
			if (!candidates[i].empty() && ::PathFileExistsW(candidates[i].c_str()))
			{
				return candidates[i];
			}
		}
	...
	static void open_path_location(HWND hWnd, const std::wstring& path, const wchar_t* error_text)
	{
		if (path.empty())
		{
			::MessageBoxW(hWnd, error_text, L"MMEffect", MB_OK | MB_ICONINFORMATION);
			return;
		}
		if (::PathFileExistsW(path.c_str()))
		{
			const std::wstring argument = std::wstring(L"/select,\"") + path + L"\"";
			::ShellExecuteW(hWnd, L"open", L"explorer.exe", argument.c_str(), NULL, SW_SHOWNORMAL);
		}
		else
		{
			wchar_t folder[MAX_PATH] = {};
			wcsncpy_s(folder, path.c_str(), _TRUNCATE);
			::PathRemoveFileSpecW(folder);
			::ShellExecuteW(hWnd, L"open", folder, NULL, NULL, SW_SHOWNORMAL);
		}
	}

	static void open_mme_object_location(HWND hWnd, HWND hList)
	{
		open_path_location(hWnd, resolve_selected_mme_item_path(hList), current_mme_ui_text().open_object_error);
	}

	static void open_mme_effect_location(HWND hWnd, HWND hList)
	{
		open_path_location(hWnd, resolve_effect_path(hList), current_mme_ui_text().open_effect_error);
	}

	static void set_window_text_safe(HWND hWnd, const wchar_t* text)
	{
		if (hWnd && text)
		{
	...
	static bool is_mme_context_menu(HMENU menu)
	{
		if (!menu) return false;
		const int count = ::GetMenuItemCount(menu);
		if (count == 3 && ::GetSubMenu(menu, 0) && ::GetSubMenu(menu, 1) && ::GetSubMenu(menu, 2))
		{
			wchar_t a[256] = {};
			wchar_t b[256] = {};
			wchar_t c[256] = {};
			::GetMenuStringW(menu, 0, a, sizeof(a) / sizeof(a[0]), MF_BYPOSITION);
			::GetMenuStringW(menu, 1, b, sizeof(b) / sizeof(b[0]), MF_BYPOSITION);
			::GetMenuStringW(menu, 2, c, sizeof(c) / sizeof(c[0]), MF_BYPOSITION);
			if ((_wcsicmp(a, L"File(&F)") == 0 || _wcsicmp(a, L"文件(&F)") == 0 || _wcsicmp(a, L"ファイル(&F)") == 0)
				&& (_wcsicmp(b, L"Edit(&E)") == 0 || _wcsicmp(b, L"编辑(&E)") == 0 || _wcsicmp(b, L"編集(&E)") == 0)
				&& (_wcsicmp(c, L"Tool(&T)") == 0 || _wcsicmp(c, L"工具(&T)") == 0 || _wcsicmp(c, L"ツール(&T)") == 0))
			{
				return true;
			}
		}
		...
		{
			wchar_t buffer[256] = {};
			::GetMenuStringW(menu, i, buffer, sizeof(buffer) / sizeof(buffer[0]), MF_BYPOSITION);
			if (_wcsicmp(buffer, L"ファイル選択") == 0
				|| _wcsicmp(buffer, L"解除") == 0
				|| _wcsicmp(buffer, L"非表示/表示") == 0
				|| _wcsicmp(buffer, L"Reset with default(&R)") == 0
				|| _wcsicmp(buffer, L"Subset-Extract(&E)") == 0
				|| _wcsicmp(buffer, L"选择文件") == 0
				|| _wcsicmp(buffer, L"批量操作选中项") == 0)
			{
				return true;
			}
		}
	...
	static bool menu_has_command(HMENU menu, UINT command)
	{
		const int count = ::GetMenuItemCount(menu);
		for (int i = 0; i < count; ++i)
		{
	...
	static int find_menu_position_by_text(HMENU menu, const wchar_t* text1, const wchar_t* text2 = NULL)
	{
		if (!menu) return -1;
		const int count = ::GetMenuItemCount(menu);
		for (int i = 0; i < count; ++i)
		{
			wchar_t buffer[256] = {};
			::GetMenuStringW(menu, i, buffer, sizeof(buffer) / sizeof(buffer[0]), MF_BYPOSITION);
			if ((text1 && _wcsicmp(buffer, text1) == 0) || (text2 && _wcsicmp(buffer, text2) == 0))
			{
				return i;
			}
		}
	...
	static void set_menu_text_by_command(HMENU menu, UINT command, const wchar_t* text)
	{
		if (!menu || !text) return;
		MENUITEMINFOW info = {};
		...
		::SetMenuItemInfoW(menu, command, FALSE, &info);
	}

	static void set_menu_text_by_position(HMENU menu, UINT position, const wchar_t* text)
	{
		if (!menu || !text) return;
		MENUITEMINFOW info = {};
		info.cbSize = sizeof(info);
	...
	static void hook_mme_main_menu()
	{
		if (!g_hWnd || !g_hMenu) return;

		const MMEUiText& text = current_mme_ui_text();
		const int mme_pos = find_menu_position_by_text(g_hMenu, L"MMEffect", L"MM效果");
		if (mme_pos < 0) return;

		g_hMMEMenu = ::GetSubMenu(g_hMenu, mme_pos);
		if (!g_hMMEMenu) return;

		if (!g_hMMELanguageMenu)
		{
			g_hMMELanguageMenu = ::CreatePopupMenu();
			::AppendMenuW(g_hMMELanguageMenu, MF_STRING, IDC_MENU_MME_LANGUAGE_ZH_CN, text.menu_language_chinese);
			::AppendMenuW(g_hMMELanguageMenu, MF_STRING, IDC_MENU_MME_LANGUAGE_SOURCE, text.menu_language_source);
		}

		if (!g_isMMELanguageMenuAttached)
		{
			::AppendMenuW(g_hMMEMenu, MF_SEPARATOR, 0, NULL);
			::AppendMenuW(g_hMMEMenu, MF_POPUP, reinterpret_cast<UINT_PTR>(g_hMMELanguageMenu), text.menu_language);
			g_isMMELanguageMenuAttached = true;
		}
	}

	static void update_mme_main_menu_language()
	{
		if (!g_hMenu || !g_hMMEMenu) return;
		const MMEUiText& text = current_mme_ui_text();

		const int mme_pos = find_menu_position_by_text(g_hMenu, L"MMEffect", L"MM效果");
		if (mme_pos >= 0)
		{
			set_menu_text_by_position(g_hMenu, mme_pos, text.main_menu_title);
		}

		set_menu_text_by_command(g_hMMEMenu, 40005, text.menu_effect_mapping);
		set_menu_text_by_command(g_hMMEMenu, 40020, text.menu_enable_effect);
		set_menu_text_by_command(g_hMMEMenu, 40006, text.menu_enable_effect);
		set_menu_text_by_command(g_hMMEMenu, 40001, text.menu_auto_reload);
		set_menu_text_by_command(g_hMMEMenu, 40022, text.menu_auto_save_mapping);
		set_menu_text_by_command(g_hMMEMenu, 40004, text.menu_reload_all);
		set_menu_text_by_command(g_hMMEMenu, 40002, text.menu_log_window);
		set_menu_text_by_command(g_hMMEMenu, 40003, text.menu_about);

		if (g_hMMELanguageMenu)
		{
			set_menu_text_by_position(g_hMMELanguageMenu, 0, text.menu_language_chinese);
			set_menu_text_by_position(g_hMMELanguageMenu, 1, text.menu_language_source);
			const int menu_count = ::GetMenuItemCount(g_hMMEMenu);
			const int lang_pos = menu_count > 0 ? menu_count - 1 : -1;
			if (lang_pos >= 0)
			{
				set_menu_text_by_position(g_hMMEMenu, lang_pos, text.menu_language);
			}
			::CheckMenuRadioItem(
				g_hMMELanguageMenu,
				IDC_MENU_MME_LANGUAGE_ZH_CN,
				IDC_MENU_MME_LANGUAGE_SOURCE,
				g_mme_ui_language == MMEUiLanguage::Chinese ? IDC_MENU_MME_LANGUAGE_ZH_CN : IDC_MENU_MME_LANGUAGE_SOURCE,
				MF_BYCOMMAND);
		}
	...
	static void update_mme_popup_menu_language(HMENU menu)
	{
		if (!menu) return;
		const MMEUiText& text = current_mme_ui_text();
		const int count = ::GetMenuItemCount(menu);
		if (count == 3 && ::GetSubMenu(menu, 0) && ::GetSubMenu(menu, 1) && ::GetSubMenu(menu, 2))
		{
			set_menu_text_by_position(menu, 0, text.popup_root_file);
			set_menu_text_by_position(menu, 1, text.popup_root_edit);
			set_menu_text_by_position(menu, 2, text.popup_root_tool);
		}

		set_menu_text_by_command(menu, 40009, text.popup_open);
		set_menu_text_by_command(menu, 40010, text.popup_save);
		set_menu_text_by_command(menu, 40016, text.popup_open_by_model);
		set_menu_text_by_command(menu, 40017, text.popup_save_by_model);
		set_menu_text_by_command(menu, 40011, text.popup_close);
		set_menu_text_by_command(menu, 40007, text.select_file);
		set_menu_text_by_command(menu, 40012, text.remove);
		set_menu_text_by_command(menu, 40013, text.hide_show);
		set_menu_text_by_command(menu, 40008, text.popup_reset_default);
		set_menu_text_by_command(menu, 40006, text.popup_subset_extract);
		set_menu_text_by_command(menu, 40014, text.popup_disassemble);
		set_menu_text_by_command(menu, 40028, text.popup_skip_validation);
		...
		for (int i = 0; i < count; ++i)
		{
			...
			if (sub)
			{
				update_mme_popup_menu_language(sub);
			}
		}
	}

	static void append_mme_context_menu_items(HMENU menu)
	{
		if (!menu || menu_has_command(menu, IDM_MME_OPEN_OBJECT)) return;
		const MMEUiText& text = current_mme_ui_text();
		::AppendMenuW(menu, MF_SEPARATOR, 0, NULL);
		::AppendMenuW(menu, MF_STRING, IDM_MME_OPEN_OBJECT, text.open_object);
		::AppendMenuW(menu, MF_STRING, IDM_MME_OPEN_EFFECT, text.open_effect);
	}

	static void update_mme_dialog_language(HWND hWnd)
	{
		if (!hWnd) return;
		const MMEUiText& text = current_mme_ui_text();
		set_window_text_safe(hWnd, text.dialog_title);
		set_window_text_safe(::GetDlgItem(hWnd, 1004), text.main_render_target);
		set_window_text_safe(::GetDlgItem(hWnd, 1007), text.select_file);
		set_window_text_safe(::GetDlgItem(hWnd, 1008), text.remove);
		set_window_text_safe(::GetDlgItem(hWnd, 1011), text.hide_show);
		set_window_text_safe(::GetDlgItem(hWnd, 1006), text.bulk_action);
		set_window_text_safe(::GetDlgItem(hWnd, IDOK), text.ok);
		set_window_text_safe(::GetDlgItem(hWnd, IDCANCEL), text.cancel);
		set_window_text_safe(::GetDlgItem(hWnd, IDC_MME_UPDATE), text.refresh);
	}

	static void set_mme_ui_language(MMEUiLanguage language)
	{
		if (g_mme_ui_language == language) return;
		g_mme_ui_language = language;
		update_mme_main_menu_language();
		if (g_hMMEDialog && ::IsWindow(g_hMMEDialog))
		{
			update_mme_dialog_language(g_hMMEDialog);
		}
	}

	static bool mme_change_tab_page(HWND hTab, int direction)
	{
		if (!hTab || direction == 0) return false;
		const int count = TabCtrl_GetItemCount(hTab);
		if (count <= 1) return false;

		const int current = TabCtrl_GetCurSel(hTab);
		if (current < 0) return false;

		const int next = std::max(0, std::min(count - 1, current + direction));
		if (next == current) return false;

		NMHDR hdr = {};
		hdr.hwndFrom = hTab;
		hdr.idFrom = static_cast<UINT_PTR>(::GetDlgCtrlID(hTab));
		hdr.code = TCN_SELCHANGING;
		LRESULT result = ::SendMessageW(::GetParent(hTab), WM_NOTIFY, hdr.idFrom, reinterpret_cast<LPARAM>(&hdr));
		if (result != 0)
		{
			return false;
		}
		...
		::SendMessageW(::GetParent(hTab), WM_NOTIFY, hdr.idFrom, reinterpret_cast<LPARAM>(&hdr));
		::InvalidateRect(::GetParent(hTab), NULL, TRUE);
		return true;
	}

	static bool handle_mme_mouse_wheel(WPARAM wParam)
	{
		if (!g_hMMETab || !::IsWindow(g_hMMETab))
		{
			return false;
		}
		...
		return mme_change_tab_page(g_hMMETab, delta < 0 ? 1 : -1);
	}

	static bool is_mme_mapping_dialog(HWND hWnd)
	{
		if (!hWnd || !::IsWindow(hWnd)) return false;
		wchar_t class_name[64] = {};
		::GetClassNameW(hWnd, class_name, sizeof(class_name) / sizeof(class_name[0]));
		if (_wcsicmp(class_name, k_mme_dialog_class) != 0) return false;

		HWND hTab = ::GetDlgItem(hWnd, IDC_MME_TAB);
		HWND hList = ::GetDlgItem(hWnd, IDC_MME_LIST);
		if (!hTab || !hList) return false;

		wchar_t tab_class[64] = {};
		wchar_t list_class[64] = {};
		::GetClassNameW(hTab, tab_class, sizeof(tab_class) / sizeof(tab_class[0]));
		::GetClassNameW(hList, list_class, sizeof(list_class) / sizeof(list_class[0]));
		return _wcsicmp(tab_class, k_mme_tab_class) == 0 && _wcsicmp(list_class, k_mme_list_class) == 0;
	}
}
...
static LRESULT CALLBACK mmeTabWndProc(HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
	if (msg == WM_MOUSEWHEEL && handle_mme_mouse_wheel(wParam))
	{
		return 0;
	}
	if (msg == WM_NCDESTROY)
	{
		g_hMMETab = NULL;
		g_originalMMETabProc = NULL;
	}
	return CallWindowProc(reinterpret_cast<WNDPROC>(g_originalMMETabProc), hWnd, msg, wParam, lParam);
}

static LRESULT CALLBACK mmeListWndProc(HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
	if (msg == WM_MOUSEWHEEL && handle_mme_mouse_wheel(wParam))
	{
		return 0;
	...
	{
		g_hMMEList = NULL;
		g_originalMMEListProc = NULL;
	}
	return CallWindowProc(reinterpret_cast<WNDPROC>(g_originalMMEListProc), hWnd, msg, wParam, lParam);
}

static LRESULT CALLBACK mmeDialogWndProc(HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
	switch (msg)
	{
	case WM_COMMAND:
		...
		{
		case IDC_MENU_MME_LANGUAGE_ZH_CN:
			set_mme_ui_language(MMEUiLanguage::Chinese);
			return 0;
		case IDC_MENU_MME_LANGUAGE_SOURCE:
			set_mme_ui_language(MMEUiLanguage::Source);
			return 0;
		}
		break;
	case WM_INITMENUPOPUP:
		{
			...
			if (is_mme_context_menu(menu))
			{
				update_mme_popup_menu_language(menu);
			}
		}
		break;
	case WM_MOUSEWHEEL:
		if (handle_mme_mouse_wheel(wParam))
		{
			return 0;
		}
		break;
	case WM_NCDESTROY:
		...
		g_hMMETab = NULL;
		g_hMMEList = NULL;
		g_originalMMEDialogProc = NULL;
		g_originalMMETabProc = NULL;
		g_originalMMEListProc = NULL;
		break;
	}
	return CallWindowProc(reinterpret_cast<WNDPROC>(g_originalMMEDialogProc), hWnd, msg, wParam, lParam);
}

static void hook_mme_dialog(HWND hWnd)
{
	if (!is_mme_mapping_dialog(hWnd)) return;
	if (g_hMMEDialog == hWnd && g_originalMMEDialogProc) return;

	g_hMMEDialog = hWnd;
	g_hMMETab = ::GetDlgItem(hWnd, IDC_MME_TAB);
	g_hMMEList = ::GetDlgItem(hWnd, IDC_MME_LIST);

	if (g_hMMEDialog && !g_originalMMEDialogProc)
	{
		g_originalMMEDialogProc = ::GetWindowLongPtr(g_hMMEDialog, GWLP_WNDPROC);
		::SetWindowLongPtr(g_hMMEDialog, GWLP_WNDPROC, reinterpret_cast<LONG_PTR>(mmeDialogWndProc));
	}
	if (g_hMMETab && !g_originalMMETabProc)
	{
		g_originalMMETabProc = ::GetWindowLongPtr(g_hMMETab, GWLP_WNDPROC);
		::SetWindowLongPtr(g_hMMETab, GWLP_WNDPROC, reinterpret_cast<LONG_PTR>(mmeTabWndProc));
	}
	if (g_hMMEList && !g_originalMMEListProc)
	{
		g_originalMMEListProc = ::GetWindowLongPtr(g_hMMEList, GWLP_WNDPROC);
		::SetWindowLongPtr(g_hMMEList, GWLP_WNDPROC, reinterpret_cast<LONG_PTR>(mmeListWndProc));
	}

	update_mme_dialog_language(hWnd);
}

static BOOL CALLBACK enumMMEWindowsProc(HWND hWnd, LPARAM lParam)
{
	DWORD process_id = 0;
	::GetWindowThreadProcessId(hWnd, &process_id);
	if (process_id == ::GetCurrentProcessId())
	{
		hook_mme_dialog(hWnd);
		if (g_hMMEDialog)
		{
			return FALSE;
		}
...
static void overrideGLWindow()
{
	...
	{
		g_hMMEMenu = NULL;
		g_hMMELanguageMenu = NULL;
		g_isMMELanguageMenuAttached = false;
	}
	if (!g_hMMEDialog || !::IsWindow(g_hMMEDialog))
	{
		g_hMMEDialog = NULL;
		...
		g_originalMMETabProc = NULL;
		g_originalMMEListProc = NULL;
		EnumWindows(enumMMEWindowsProc, 0);
	}
	setMyMenu();
	hook_mme_main_menu();
	update_mme_main_menu_language();
